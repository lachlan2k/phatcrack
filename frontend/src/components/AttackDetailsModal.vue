...
const canStop = computed(() => {
  return props.attack.jobs.some(
    (x) =>
      x.runtime_data.status == JobStatusStarted || x.runtime_data.status == JobStatusAwaitingStart
  )
})
  ...
  <Modal v-model:isOpen="isOpen">
        ...
        <tr v-for="job in attack.jobs" :key="job.id">
          ...
          <td style="min-width: 130px">
            ...
            <div
              ...
              v-else-if="job.runtime_data.stop_reason == JobStopReasonUserStopped"
            >
              Job stopped
            </div>
            ...
            <p
              ...
              class="pt-1 font-mono"
              v-if="job.runtime_data.error_string != ''"
            >
              {{ job.runtime_data.error_string }}
            </p>
          </td>
          ...
          <td v-if="job.runtime_summary.stopped_time > 0">
            {{
              timeBetween(
                job.runtime_summary.started_time * 1000,
                job.runtime_summary.stopped_time * 1000
              )
            }}
          </td>
          <td v-else>-</td>
    ...
    <div class="mt-8 flex flex-row justify-center">
      <div class="join">
        <button
          @click="() => start()"
          class="btn btn-success join-item btn-sm"
          v-if="attack.jobs.length == 0"
        >
