...
export function loginWithOIDCCallback(
  querystring: string
): Promise<AuthLoginResponseDTO> {
  return client
    .post("/api/v1/auth/login/oidc/callback" + querystring)
    .then((res) => res.data);
}
...
export function finishMFAEnrollment(
  ...
  return client
    .post("/api/v1/auth/mfa/finish-enrollment?method=MFATypeWebAuthn", {
      ...
      response: {
        ...
        attestationObject: urlSafeB64Encode(
