...
<template>
  <main class="w-full p-4">
    ...
    <small class="text-sm pl-0.5 mt-1 mb-2">Hint: when you create a hashlist, all hashes are searched automatically for you</small>
    <div class="mt-3 flex flex-wrap gap-6">
      <div class="card bg-base-100 shadow-xl">
        <div class="card-body flex-row gap-3">
              <div class="min-w-[400px]">
                <h2 class="text-xl font-semibold mb-2">Enter hashes to search (one per-line)</h2>
                <HashesInput v-model="hashesInput" />
                <button class="w-full mt-2 btn btn-primary" :disabled="isLoading" @click="doSearch">
                ...
                <table  class="compact-table table table-sm w-full">
                    ...
                    <tbody>
                      <tr v-for="(result, i) in results.results" :key="[i, result.hash, result.hash_type, result.plaintext_hex].join('|')">
                        ...
                        <td v-if="result.found">
                          {{  getHashTypeName(result.hash_type) }} - {{ result.hash_type }}
                        </td>
                        <td v-else>
                          Unknown
                        </td>
